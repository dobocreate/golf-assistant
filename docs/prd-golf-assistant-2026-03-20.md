...
- **プレー前（PCブラウザ）:** Claudeサブスクリプションを活用し、コース調査・YouTube動画分析・傾向分析を手動対話で実施。結果をアプリのデータベースに蓄積する。
- **プレー中（スマホブラウザ）:** 蓄積済みデータ（プレーヤー特性、コース情報、ナレッジ、過去傾向）をシステムプロンプトとして事前構築。ユーザーは状況をボタン/音声で入力するだけで、Google Gemini API（Flash-Lite）がコンテキストに基づいたアドバイスを生成する。
- **プレー後（PCブラウザ）:** スコアデータや反省メモをClaudeサブスクリプションで分析し、次回に向けた改善点を蓄積する。
...
**Description:**
ラウンド前に、AIがコース情報とユーザー特性を踏まえた事前アドバイスを生成する。※MVP期間中はClaudeサブスクリプションによる手動運用で代替。アプリ内実装はPost-MVPで検討。
...
**Description:**
蓄積されたラウンドデータを基に、AIがスコア傾向やミスパターンを分析し、改善提案を行う。※MVP期間中はClaudeサブスクリプションで手動分析。アプリはスコアデータのエクスポート機能のみ提供。
...
**Description:**
個人〜少人数での運用を前提とし、月額の運用コストを最小限に抑える。全機能をクラウド上の単一アプリケーションで提供し、APIコストが発生するのはプレー中のリアルタイムアドバイスのみとする。
...
|---------|---------|-----------|--------|
| プレー前: コース調査・分析 | クラウド（PCブラウザ） | Claudeサブスク（手動対話） | サブスク内 |
| プレー前: コンテキスト構築 | クラウド（PCブラウザ） | アプリがデータを自動集約 | ¥0 |
| プレー中: AIアドバイス | クラウド（スマホブラウザ） | Gemini API Flash-Lite（事前構築コンテキスト＋状況入力） | 無料枠内 or 約5円/ラウンド |
| プレー中: スコア記録 | クラウド（スマホブラウザ） | APIなし（データ保存のみ） | ¥0 |
| プレー後: 振り返り・分析 | クラウド（PCブラウザ） | Claudeサブスク（手動対話） | サブスク内 |
| YouTube分析 | クラウド（PCブラウザ） | Claudeサブスク（手動対話） | サブスク内 |
...
**Acceptance Criteria:**
- [ ] 全機能がクラウド上の単一アプリケーションで動作する
- [ ] ホスティングは無料枠または月額1,000円以内
...
### フロー4: YouTube動画分析（アプリ外・Claude手動）
...
6. 全機能はクラウド上で動作し、PC/スマホの両ブラウザからアクセスする
...
| 1.5 | 2026-03-21 | kishida | レビュー反映: 全クラウド前提に統一、FR-002をShould Haveに修正、FR-011/015にPost-MVP注記追加 |
...
| Priority | Count | FRs |
|----------|-------|-----|
| **Must Have** | 9 | FR-001, FR-004, FR-005, FR-006, FR-008, FR-009, FR-010, FR-014, FR-016, FR-017 |
| **Should Have** | 6 | FR-002, FR-003, FR-007, FR-011, FR-015 |
