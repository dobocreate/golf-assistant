...
| # | タスク | 状態 |
|---|--------|------|
| 1 | ホールインポートCSV様式の説明追加とサンプルダウンロード | completed（PR #18 でマージ済み） |
| 2 | スコア入力画面の再設計（楽天ゴルフスコアアプリ参考） | completed（PR #18 でマージ済み） |
| 3 | ラウンド完了後のスコア編集機能 | completed（PR #18 でマージ済み） |
| 4 | AIアドバイス画面の改善（ホール選択・傾斜入力） | completed（PR #18 でマージ済み） |
