...
| v5 | 設計適合性監査（Claude + Codex）10件の乖離修正・仕様明確化 |

### v5 での主な変更点

1. **version++は「データ変更時のみ」を明確化** — flushのたびにインクリメントすると未同期判定が増殖する。collectDataが前回値と同一の場合はversion++しない
2. **Client Shellのwrite data復元を明記** — read cacheだけでなくscores/shots/companionsもIndexedDBから復元しScoreInputに渡す
3. **sessionStorage移行の独立性を明記** — 各データタイプ（scores/shots/companions）を独立に移行判定。scoresが移行済みでもshots/companionsは個別にチェック
4. **保存ボタンのdisable条件を明確化** — オーケストレーターの`isProcessing`を使用。旧`useTransition`の`isPending`は使わない
5. **IndexedDBフォールバックの接続を必須化** — `checkIndexedDBAvailability()`を初期化時に呼び出し、結果をSyncStatusIndicatorに接続
6. **clientIdのサーバー送信を必須化** — replace RPCのペイロードにclientIdを含め、DB側でもclient_idカラムを保持。リトライ重複排除に使用
7. **旧保存経路（sessionStorage直書き、直接Server Action呼出）の完全廃止** — score-input.tsxのsessionStorage使用、companion-score-editorの直接Server Action呼出を廃止
...
データ変更（ユーザー入力）
  ...
  │     データが前回保存値と異なる場合のみ version++ をインクリメント
  │     ※ 未変更データのflushではversionを変えない（未同期判定の増殖防止）
  │
...
export function ScoreClientShell({ serverData, roundId }: Props) {
  ...
  useEffect(() => {
    ...
    // サーバーデータなし → IndexedDBから読み取り+書き込みデータを復元
    (async () => {
      const cached = await offlineStore.loadReadData();      // 読み取りデータ（holes, gamePlans等）
      const scores = await offlineStore.loadScoresLocal();    // 書き込みデータ: スコア
      const shots = await offlineStore.loadShotsLocal();      // 書き込みデータ: ショット
      const companions = await offlineStore.loadCompanionsLocal(); // 書き込みデータ: 同伴者
      if (cached && scores) {
        setData(buildDataFromCache(cached, scores, shots, companions));
...
>
> **v5での必須化:** clientIdは`replaceShotsForHole()`のペイロードと`/api/sync`の
> リクエストボディに必ず含める。`replace_shots_for_hole` RPCもclient_idパラメータを
> 受け取り、shotsテーブルのclient_idカラムに保存する。これによりリトライ時の
> 重複挿入を防止する。
...
// 初期化時に必ず呼び出し、結果をSyncStatusIndicatorのidbAvailableに接続する
...
操作キュー実行中（`orchestrator.isProcessing === true`）はボタンをdisabled表示。

> **v5での明確化:** disable条件はオーケストレーターの`isProcessing`を使用する。
> 旧実装で使われていた`useTransition`の`isPending`はオーケストレーター導入後は
> 無関係であり、使用してはならない。
...
async function migrateFromSessionStorage(
  ...
  // 各データタイプを独立に移行判定する（scoresが移行済みでもshots/companionsは個別チェック）
  await migrateScoresFromSessionStorage(roundId, serverScores);
  await migrateShotsFromSessionStorage(roundId);
  await migrateCompanionsFromSessionStorage(roundId);
}

async function migrateScoresFromSessionStorage(
  roundId: string,
  serverScores: Score[],
): Promise<void> {
  const idbData = await loadScoresLocal();
  if (idbData) return;  // このデータタイプは移行済み

  const ssScores = getSession<Map<number, Score>>(roundScoresKey(roundId));
  if (!ssScores || ssScores.size === 0) return;
...
// migrateShotsFromSessionStorage, migrateCompanionsFromSessionStorage も同様に
// 1. 個別にIDB移行済みかチェック
// 2. サーバーデータとの比較で差分判定
// 3. 型はMap<number, LocalShot[]> / Map<number, HoleInputs> に変換して保存
```
...
>
> **v5での明確化:**
> - 各データタイプ（scores/shots/companions）を**独立に**移行判定する。
>   scoresがIndexedDB移行済みでもshots/companionsは未移行の可能性がある
> - shots/companionsにも**サーバーデータとの比較**を適用する
> - companionsの型は`Map<number, HoleInputs>`に統一する
>   （sessionStorage上の`CompanionHoleInput[]`からの変換が必要）
